...
- ${promptLanguageRule(language, "Respond in the same language as the project mission/name (Korean if Korean, English if English)")}`;
  ...
  router.get("/:goalId/squash-preview", (req, res) => {
    ...
    const commitPrefix = typeof wr?.commitType === "string" && /^(feat|fix|update|docs|refactor|chore|test)$/.test(wr.commitType) ? `${wr.commitType}: ` : "";
    const commitMessage = `${commitPrefix}${goal.title || goal.description}\n\nTasks:\n${taskBullets}\n\nGenerated by Crewdeck (Goal-as-Unit)`;
