...
describe("Goal-as-Unit E2E — Full Auto worktree 기록", () => {
  ...
  it("QA와 acceptance_script PASS 후 pending_approval과 squash preview를 남기고 승인 전 base branch를 건드리지 않는다", { timeout: 60_000 }, async () => {
    ...
    try {
      ...
      // squash 후에도 살아남는 추적 trailer (Refs=goal id, Assisted-by=오케스트레이터 귀속)
      expect(preview.commitMessage).toContain(`Refs: goal-${goalId}`);
      expect(preview.commitMessage).toContain("Assisted-by: Crewdeck");
